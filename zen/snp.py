...
def getSnapshots(snapdir):
    snapshots = sorted([
        name for name in os.listdir(snapdir)
        if os.path.isdir(os.path.join(snapdir, name)) and name.startswith("1-")
    ])
...
def updateSnapshot():
    ...
    snapdir = os.path.expanduser(
        os.path.join("~", ".local", "share", appname, network, "snapshots")
    )
    snapshots = getSnapshots(snapdir)

    if not os.system(
        'ark snapshot:dump --blocks %(snapshot)s' % {"snapshot": snapshots[-1]}
    ):
        for snapshot in snapshots:
            os.system('rm -rf "%s"' % os.path.join(snapdir, snapshot))
        zen.misc.notify("Blockchain snapped !")
    os.system(
        'cd ~ && tar cf ~/last-snapshot.tar '
        '.local/share/ark-core/%(network)s/snapshots/1-*' %
        {"network": network}
    )
...
def rebuildFromZero():
    ...
    snapdir = os.path.expanduser(
        os.path.join(
            "~", ".local", "share", appname, root["name"], "snapshots"
        )
    )
    snapshots = getSnapshots(snapdir)
    ...
    os.system(
        'ark snapshot:restore --blocks %(snapshot)s --truncate' %
